try {
  ...
  async function initializeDB() {
    if (Object.keys(collections).length > 0) return; // Already initialized
  ...
  // Routes - ALL ROUTES DEFINED HERE (not in async function)
  ...
  app.post("/requests", async (req, res) => {
    try {
      ...
      if (alreadyRequested) {
        return res
          .status(400)
          .send({ message: "You already sent this request" });
      }
